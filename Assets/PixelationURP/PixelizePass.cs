...
public class PixelizePass : ScriptableRenderPass
{
    ...
    private int i = 0;
    float timer = 0;
    private void Pixelating() {
        settings.UpdateRange();

        settings.screenHeight = settings.screenHeightRange[i];
        
        timer -= Time.fixedUnscaledDeltaTime;
        if(timer <= 0) {
            timer = settings.pixelatingSpeed;
            i++;
        }

        if(i >= settings.screenHeightRange.Length)
            i = 0;
    }
}